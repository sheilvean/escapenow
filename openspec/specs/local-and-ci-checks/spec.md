...
tests, OpenSpec artifact validation, agent-configuration validation, continuous-integration
workflow validation, and the repository tooling tests. Each stage SHALL report its command, its
...
secrets, using tools pinned by version and verified by checksum where they are downloaded.
